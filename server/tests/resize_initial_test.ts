...
Deno.test("initial resize before R frames — stale entry bug", withTestHarness(async (t, { rClient, browser }) => {
 ...
 */
Deno.test("two resizes before first frame — replay must be tagged", withTestHarness(async (t, { rClient, browser }) => {
  await rClient.waitForWelcome();
...
Deno.test("resize after first frame — correctly tags replay", withTestHarness(async (t, { rClient, browser }) => {
  await rClient.waitForWelcome();
