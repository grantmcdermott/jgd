...
Deno.test("deferred resize entry survives collapse from later resize", withTestHarness(async (t, { rClient, browser }) => {
...
Deno.test("rapid resizes after first frame — all replays tagged", withTestHarness(async (t, { rClient, browser }) => {
  await rClient.waitForWelcome();
